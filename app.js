...
// API Helper with resilient client-side fallback
...
async function fallbackClientApi(path, options = {}) {
  try {
    const data = await getStaticData();
    const cleanPath = path.split("?")[0].replace(/\/$/, "");

    if (cleanPath === "/api/overview") {
      const wanted = (data.suspects || []).filter((s) => s.status === "Wanted").length;
      const openFirs = (data.firs || []).filter((f) => ["Open", "Under investigation", "Chargesheet filed"].includes(f.status)).length;
      const totalLoss = (data.firs || []).reduce((sum, f) => sum + (f.loss_estimate_inr || 0), 0);
      const totalTransferred = (data.transactions || []).reduce((sum, t) => sum + (t.amount_inr || 0), 0);
      return {
        suspects: data.suspects.length,
        firs: data.firs.length,
        ...
        total_transferred_inr: totalTransferred,
        meta: data.meta || {},
        data_mtime: Date.now(),
      };
    }

    if (cleanPath === "/api/suspects") return data.suspects || [];
    if (cleanPath === "/api/firs") return data.firs || [];
    if (cleanPath === "/api/vehicles") return data.vehicles || [];
    if (cleanPath === "/api/calls") return data.calls || [];
    if (cleanPath === "/api/transactions") return data.transactions || [];

    if (cleanPath === "/api/intel") {
      return {
        total_suspects: (data.suspects || []).length,
        total_firs: (data.firs || []).length,
        total_transactions: (data.transactions || []).length,
        summary: "Pan-India intelligence correlation active.",
      };
    }

    if (cleanPath.startsWith("/api/suspects/")) {
      const sId = cleanPath.split("/").pop();
      const suspect = (data.suspects || []).find((s) => s.id === sId) || (data.suspects && data.suspects[0]) || {};
      const firs = (data.firs || []).filter((f) => (suspect.fir_ids || []).includes(f.id));
      const vehicles = (data.vehicles || []).filter((v) => v.owner_id === sId || (suspect.vehicle_ids || []).includes(v.id));
      const calls = (data.calls || []).filter((c) => c.from_suspect === sId || c.to_suspect === sId);
      const txns = (data.transactions || []).filter((t) => t.from_suspect === sId || t.to_suspect === sId);
      const associates = (data.suspects || []).filter((s) => (suspect.known_associates || []).includes(s.id));
      return {
        suspect,
        firs,
        vehicles,
    ...
    if (cleanPath === "/api/graph") {
      const nodes = [];
      const links = [];
      (data.suspects || []).forEach((s) => {
        nodes.push({ id: s.id, label: s.name, type: "suspect", risk: s.risk, status: s.status, city: s.city });
        (s.known_associates || []).forEach((assocId) => {
          links.push({ source: s.id, target: assocId, type: "associate" });
        });
        (s.accounts || []).forEach((acc) => {
          nodes.push({ id: acc, label: acc, type: "account", city: s.city });
          links.push({ source: s.id, target: acc, type: "account" });
        });
      });
      (data.firs || []).forEach((f) => {
        nodes.push({ id: f.id, label: f.id, type: "fir", loss: f.loss_estimate_inr });
        (f.accused_ids || []).forEach((accusedId) => {
          links.push({ source: accusedId, target: f.id, type: "fir_link" });
        });
      });
      return { nodes, links };
    }
    ...
    if (cleanPath === "/api/geo") {
      const coords = {
        S001: [28.5398, 77.2384],
        S002: [28.3392, 77.3245],
        S003: [28.4947, 77.0895],
        S004: [28.5677, 77.2433],
        S005: [28.4124, 77.3178],
        S006: [28.6562, 77.231],
        S007: [28.6692, 77.4538],
        S008: [28.6304, 77.2773],
        S009: [18.956, 72.839],
        S010: [26.9075, 75.7396],
      };
      const markers = [];
      (data.suspects || []).forEach((s) => {
        const c = coords[s.id] || [28.6139, 77.209];
        markers.push({
          id: s.id,
          kind: "suspect",
          lat: c[0],
          lng: c[1],
          label: s.name,
          detail: `${s.role || "Operative"} • ${s.city || "NCR"}`,
          status: s.status,
        });
      });
      return markers;
    ...
    if (cleanPath === "/api/records" && options.method === "POST") {
      const body = typeof options.body === "string" ? JSON.parse(options.body) : options.body || {};
      const rType = body.type || "suspects";
      const record = body.data || {};
      record.id = record.id || (rType === "suspects" ? `S0${(data.suspects || []).length + 1}` : `REC-${Date.now().toString().slice(-4)}`);
      if (!data[rType]) data[rType] = [];
      data[rType].push(record);
      return { success: true, record: record };
    }

    return {};
  } catch (e) {
    console.warn("[LinkTrace Fallback Engine Error]:", e.message);
    return {};
  }
}
...
async function api(path, options = {}) {
  try {
    ...
    return await fallbackClientApi(path, options);
  }
}
